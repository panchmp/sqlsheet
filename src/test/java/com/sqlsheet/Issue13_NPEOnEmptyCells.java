...
public class Issue13_NPEOnEmptyCells {
    ...
    private final String[] columnNames =
            {"String", "Date", "Boolean", "Double", "String", "Date", "Boolean", "Double"};
    private final Object[][] cellValues = {
            {"Row1", new java.util.Date(), Boolean.TRUE, 1d, "Test", new java.util.Date(),
                    Boolean.FALSE, Math.PI},
            {"Row2", new java.util.Date(), Boolean.FALSE, 2d, "Test", new java.util.Date(),
                    Boolean.TRUE, 2 * Math.PI},
            {"Row3", new java.util.Date(), Boolean.TRUE, 3d, "Test", new java.util.Date(),
                    Boolean.FALSE, 3 * Math.PI},
            {"Row4", new java.util.Date(), Boolean.TRUE, 1d, "Test", new java.util.Date(),
                    Boolean.FALSE, Math.PI},
            {"Row5", new java.util.Date(), Boolean.FALSE, 2d, "Test", new java.util.Date(),
                    Boolean.TRUE, 2 * Math.PI},
            {"Row6", new java.util.Date(), Boolean.TRUE, 3d, "Test", new java.util.Date(),
                    Boolean.FALSE, 3 * Math.PI}
    };
    ...
    public Issue13_NPEOnEmptyCells() throws SQLException, IOException, ClassNotFoundException {
        ...
        short dateFormatIndex =
                dateFormat.getFormat(DateFormatConverter.convert(Locale.US,
        ...
        File file =
                File.createTempFile(Issue13_NPEOnEmptyCells.class.getSimpleName() + "_", ".xlsx");
        file.deleteOnExit();
        ...
        conn = DriverManager.getConnection(
                "jdbc:xls:" + file.toURI().toASCIIString() + "?headLine=" + DEFAULT_HEADLINE
                        + "&firstColumn=" + DEFAULT_FIRST_COL);
        ...
        try {
            ...
            while (rs.next()) {
                ...
                for (int c = 0; c < 4; c++) {
                    ...
                    if (!rs.wasNull()) {
                        Assertions.assertEquals(valueByColumnIndex, valueByColumnLabel,
                                "Column " + (c + 1) + " " + columnNames[c + 1] + " fails.");
                    }
                ...
                for (int c = 0; c < columnNames.length; c++) {
                    ...
                    if (r % 2 == 0) {
                        Assertions.assertTrue(rs.wasNull(),
                                "Column " + columnNames[c] + " in row " + r + " should be NULL.");
                    }
                }
                at.addRule();
                at.addRow(values);
