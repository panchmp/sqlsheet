...
public class SqlSheetParser {
  ...
  public ParsedStatement parse(String sql) throws SQLException {
    ...
    if (statement instanceof Select) {
      ...
      List<FromItem> froms = ((PlainSelect)body).getFromItems();
      if (froms == null || froms.isEmpty() || froms.size() > 1) {
        throw new SQLException(
            "Must SELECT from exactly one table from Excel sheet.");
      }
      final FromItem from = froms.get(0);
