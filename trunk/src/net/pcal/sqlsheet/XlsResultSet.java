...
public class XlsResultSet implements ResultSet {
  ...
  public XlsResultSet(HSSFWorkbook wb, HSSFSheet s) throws SQLException {
  ...
  @Deprecated
  public XlsResultSet(HSSFSheet s, boolean b)
  {
   ...
   */
  public void setOutputDateStyle(HSSFCellStyle style) {
  ...
  public boolean getBoolean(int jdbcColumn) throws SQLException {
    HSSFCell cell = getCell(jdbcColumn);
  ...
  private static Object getObject(HSSFCell cell) throws SQLException {
    try {
      ...
      if (HSSFDateUtil.isCellDateFormatted(cell)) {
        ...
        System.out.println(cell.getNumericCellValue() + "==========>" + d
            + " =========> " + sqlDate);
        return sqlDate;
      }
  ...
  public void updateShort(int jdbcColumn, short x) throws SQLException {
    HSSFCell cell = findOrCreateCell(jdbcColumn);
    if (cell != null) cell.setCellValue(x);
  }
  ...
  public void moveToInsertRow() throws SQLException {
    for (cursorSheetRow = 1; true; cursorSheetRow++) {
      HSSFRow row = sheet.getRow(cursorSheetRow);
  ...
  private void updateObject(HSSFCell cell, Object x) throws SQLException {
    if (x instanceof String) {
      ...
      cell.setCellValue(HSSFDateUtil.getExcelDate((java.util.Date)x));
      if (dateStyle != null) cell.setCellStyle(dateStyle);
  ...
  private HSSFCell findOrCreateCell(int jdbcColumn) {
    HSSFCell cell = getCell(jdbcColumn);
    if (cell == null) {
  ...
  private HSSFCell findOrCreateCell(String jdbcColumn) {
    int col = getSheetColumnNamed(jdbcColumn);
    return findOrCreateCell(col);
  }

  private HSSFCell getCell(int jdbcColumn) {
    return sheet.getRow(cursorSheetRow).getCell((short)(jdbcColumn - 1));
  }

  private HSSFCell getCell(String named) {
